...
$stmt = $mysqli->prepare("SELECT jd.uid, jd.start_time, jd.end_time, jd.location, jd.tractors, jd.bobtails, jd.drivers, jd.movers, jd.installers, jd.pctechs, jd.supervisors, jd.project_managers, jd.crew_transport, jd.electricians, jd.day_notes, jd.status, j.title AS customer_name, c.name AS contractor_name FROM job_days jd JOIN jobs j ON j.uid = jd.job_uid JOIN contractors c ON c.id = jd.contractor_id WHERE jd.work_date=? ORDER BY c.display_order, jd.start_time");
...
<body class="<?= $isMobile ? 'mobile' : 'desktop' ?>">
  ...
  <div class="nav">
    ...
    <a href="mobile_schedule.php?date=<?= $next ?>">Next Day &gt;</a>
  </div>
  ...
  <?php foreach ($jobs as $j): $uid=h($j['uid']); $status=strtolower($j['status'] ?? ''); ?>
    <button class="job-btn status-<?= h($status) ?>" onclick="toggleJob('<?= $uid ?>')">
      <?= h($j['contractor_name']) ?> - <?= h($j['customer_name']) ?> (<?= fmtTime($j['start_time']) ?>-<?= fmtTime($j['end_time']) ?>)
    </button>
